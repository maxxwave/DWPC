...
namespace programs{
    ...
    double bifurcation(){
        ...
        const double no_steps_per_period = 1.0/(integrate::Dt*stor::freq);

        std::cerr << "Nsteps = " << no_steps_per_period << std::endl;
        ...
        for(int i=0;i<500;i++){
            ...
            for (int l=0; l<200; l++){
                ...
                output << stor::V0 <<"\t" << stor::x_dw << "\t"<< stor::V << "\t" <<stor::vx << std::endl;
