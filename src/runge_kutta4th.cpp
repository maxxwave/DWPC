...
namespace integrate{
	...
	double x_t(double DWs, double phi){
		return prefac2*sin(2*phi)*DWs + stor::alpha*DWs*phi; 
	}
	double runge_kutta(){
		phi_k=stor::phi_dw;
		x_k=stor::x_dw;
		...
		k1=integrate::Dt*x_t(stor::Dw_size, phi_k);

		kp2=integrate::Dt*phi_t(stor::dEx, phi_k+0.5*kp1, stor::V);
		k2=integrate::Dt*x_t(stor::Dw_size,phi_k+0.5*k1);

		kp3=integrate::Dt*phi_t(stor::dEx, phi_k+0.5*kp2, stor::V);
		k3=integrate::Dt*x_t(stor::Dw_size,phi_k+0.5*k2);

		kp4=integrate::Dt*phi_t(stor::dEx, phi_k+kp3, stor::V);
		k4=integrate::Dt*x_t(stor::Dw_size,phi_k+k3);
		stor::vx=k4;
		stor::phi_dt=kp4;
